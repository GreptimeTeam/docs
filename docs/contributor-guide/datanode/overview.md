...
- A Region server manages the lifecycle of all `Region`s on a `Datanode` and dispatches requests to the appropriate storage engine.
- GreptimeDB supports multiple Region engines. `Mito` is the primary time-series storage engine, `Metric` stores many logical metric tables in shared Mito Regions, and `File` provides access to external files.
