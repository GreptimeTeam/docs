...
:::tip NOTE
Note: Export metrics has nothing to do with using Prometheus to export metrics generated by `greptimedb`. Learn how to export `greptimedb` metrics to Prometheus in [monitoring](./monitoring.md). Export metrics is only used as a way for `greptimedb` to export the metrics generated by itself.
:::
