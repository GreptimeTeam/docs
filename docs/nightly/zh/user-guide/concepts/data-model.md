...
GreptimeDB 使用时序表来进行数据的组织、压缩和过期管理。数据模型主要基于关系型数据库中的表模型，同时考虑到了指标（Metrics）及事件（Events）数据的特点。
...
- `Timestamp` 是日志及指标的时序数据库的基础，它表示数据生成的日期和时间。Timestamp 具有索引，所以使用 `Timestamp` 的查询具有良好的性能。一个表只能有一个 `Timestamp` 列，被称为时间索引列。
...
- `request` 是通过 `FULLTEXT` 列选项启用全文索引的字段。
...
- 表格模型易于学习，具有广泛的用户群体，我们只需引入时间索引的概念即可实现时间和日志序列。
...
- 比起 OpenTSDB 和 Prometheus 采用的单值模型，GreptimeDB 使用多值模型使其中一行数据可以具有多列数据。多值模型面向数据源建模，一个 metric 可以有用 field 表示的值。多值模型的好处是可以从数据库一次性读写多个值，而单值模型则要分成多条数据。请参考[博客](https://greptime.com/blogs/2024-05-09-prometheus)查看多值模型的详细好处。
