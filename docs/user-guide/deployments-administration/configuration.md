...
| pending_rows_batcher |                              |         | Shared experimental ordinary-table batching for opted-in HTTP ingestion protocols. Supported values for `protocols` are `influxdb`, `opentsdb`, `otlp`, `logs`, `loki`, `splunk`, `elasticsearch`, `http_sql`, and `prom`. Omitting `protocols` or setting it to an empty array disables this batching path. For `prom`, this path applies only when the metric engine is disabled; otherwise, the settings under `prom_store` apply. Streaming Flow source tables are not supported by this batching path. |
...
| `auto_flush_interval`                    | String  | `10m`          | Interval to auto flush a region if it has not flushed yet. Can be overridden per table with the [`auto_flush_interval` table option](/reference/sql/create.md#table-options).                                                                                                                                                                                                                                                                                                                       |
...
The default `auto_flush_interval` is 10 minutes. An explicit value in the Mito engine configuration overrides this default; a per-table `auto_flush_interval` overrides the engine setting.
...
## The total timeout for a Kafka request, covering sending the request and waiting for the response.
## **It's only used when the provider is `kafka`**.
timeout = "5s"
...
| `wal.timeout`                                 | String  | `5s`                         | The total timeout for a Kafka request, covering sending the request and waiting for the response.<br/>**It's only used when the provider is `kafka`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
