...
DDL events are recorded asynchronously and on a best-effort basis. For a Procedure,
execution normally has a `Running`/`Submitted` row followed by a `Done`/`Succeeded` row.
