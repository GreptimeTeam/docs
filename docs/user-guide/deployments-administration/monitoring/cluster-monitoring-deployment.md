...
This deploys a GreptimeDB Standalone instance named `${cluster-name}-monitor` to collect metrics and logs. You can verify the deployment with:

```bash
kubectl get greptimedbstandalones.greptime.io ${cluster-name}-monitor -n ${namespace}
```

The GreptimeDB Standalone instance exposes services using `${cluster-name}-monitor-standalone` as the Kubernetes Service name. You can use the following addresses to access monitoring data:
...
:::danger
The cleanup operation will remove the monitoring data stored by the GreptimeDB Standalone monitoring instance. It does not remove data from the monitored GreptimeDB cluster. Back up the monitoring data first if you need to retain it.
:::
