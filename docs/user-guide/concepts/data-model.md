...
- GreptimeDB applies the same Tag + Timestamp + Field concepts to metrics, logs, traces, and wide events. These signals can remain in separate physical tables. All signal types can be queried with SQL; metrics also support PromQL, and traces have an experimental Jaeger-compatible query API. Read [Unified observability data model](./observability-2.md) for the mapping and trade-offs.
