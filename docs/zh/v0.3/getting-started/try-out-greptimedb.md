...
我们先通过最简单的配置来开始。有关 GreptimeDB 中可用的所有配置选项的详细列表，请参考[配置文档](/v0.3/user-guide/operations/configuration.md)。
...
GreptimeDB 支持[多种协议](/v0.3/user-guide/clients/overview.md)。这里使用 MySQL 客户端示例。
...
**注意: GreptimeDB 提供了一种 schemaless 的数据写入方法，不用像使用其他协议那样手动创建表。详情请参见 [自动生成表结构](/v0.3/user-guide/write-data/overview.md#自动生成表结构)。**
...
- 如果用户使用其他协议，该表可以自动创建。请参考 [Create Table](/v0.3/user-guide/table-management#creat-table)。
- 关于创建表的 SQL 信息，请参考 [CREATE](/v0.3/reference/sql/create.md)。
- 关于数据类型，请参考[数据类型](/v0.3/reference/data-types.md)。
...
关于 `INSERT` 语句的更多信息，请参考 [INSERT](/v0.3/reference/sql/insert.md)。
...
关于 `SELECT` 语句的更多信息，请查看 [SELECT](/v0.3/reference/sql/select.md) 文件。

## Visualize data

可视化在有效利用时间序列数据方面发挥着关键作用。为了帮助用户充分利用 GreptimeDB 的各种功能，Greptime 提供了一个简单的 [dashboard](https://github.com/GreptimeTeam/dashboard)。

Dashboard 自 GreptimeDB v0.2.0 版本以来已经嵌入到 GreptimeDB 的 binary 文件中。在[启动 GreptimeDB](#安装)后，可以通过 HTTP 端点 `http://localhost:4000/dashboard` 访问 dashboard。当前版本的 dashboard 支持 MySQL 和 Python 查询，不久将支持 PromQL。
...
- [概念](/v0.3/user-guide/concepts/overview.md)
- [客户端](/v0.3/user-guide/clients/overview.md)
- [管理表](/v0.3/user-guide/table-management.md)
- [数据写入](/v0.3/user-guide/write-data/overview.md)
- [数据查询](/v0.3/user-guide/query-data/overview.md)
- [脚本-&-函数](/v0.3/user-guide/scripts-&-functions/overview.md)
- [集群](/v0.3/user-guide/cluster.md)
- [运维操作](/v0.3/user-guide/operations/overview.md)
