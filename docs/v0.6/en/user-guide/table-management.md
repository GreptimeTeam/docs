...
curl -X POST \
  ...
  -d 'sql=CREATE TABLE monitor (host STRING, ts TIMESTAMPMILLISECOND DEFAULT CURRENT_TIMESTAMP(), cpu FLOAT64 DEFAULT 0, memory FLOAT64, TIME INDEX (ts), PRIMARY KEY(host)) ENGINE=mito WITH(regions=1)' \
