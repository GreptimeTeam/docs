...
- `skip_wal`: whether to disable Write-Ahead-Log for this table. When set to `'true'`, the data written to the table will not be persisted to the write-ahead log, which can improve write throughput. However, when the process restarts, any unflushed data will be lost. Please use this feature only when the data source itself can ensure reliability. Changing `skip_wal` does not switch the table's WAL provider. You can change it from `false` to `true`, but setting it back to `false` requires a RaftEngine or Kafka WAL provider. Tables using the Noop WAL provider cannot set it to `false`; for example, tables created with `skip_wal='true'` use Noop. `UNSET` is not supported. Changing this option affects subsequent writes only; rows written while WAL was disabled are not retroactively written to WAL. It is only supported on physical tables using the `mito` or `metric` engine, and it must be set on its own: combining `skip_wal` with other table options in the same `ALTER TABLE` is rejected.
...
ALTER TABLE monitor SET 'skip_wal'='false';
