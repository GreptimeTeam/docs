...
```
TQL EXPLAIN (start, end, step) expr;
```
...
Notice that since the given query won't be actual executed, thus the triple `(start, end, step)` need not to match the actual value -- they are just placeholder. The result should be like the following:
