...
| `--internal-grpc-bind-addr <INTERNAL_GRPC_BIND_ADDR>` | internal gRPC 服务绑定地址                                                                                                                                                                                                                                                             |
| `--internal-grpc-server-addr <INTERNAL_GRPC_SERVER_ADDR>` | 该地址会通告给 metasrv，并用于来自主机外部的 internal gRPC 连接。如果留空或未设置，服务器将自动使用主机上第一个网络接口的 IP 地址，其端口号与 `internal_grpc_bind_addr` 中指定的相同； |
...
[`frontend.example.toml`](https://github.com/GreptimeTeam/greptimedb/blob/v1.1.4/config/frontend.example.toml) 位于 GreptimeDB 仓库中。`-c` 选项用于选择配置文件，详见[配置](/user-guide/deployments-administration/configuration.md)。
