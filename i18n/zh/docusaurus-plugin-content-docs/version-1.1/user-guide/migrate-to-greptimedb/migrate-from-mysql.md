...
因此需要记录失败的写入、对两个目标分别重试，并在切换前核对两边数据。重试前先确认数据确实没有落库——[append-only 表](/user-guide/manage-data/overview.md#通过创建带有-append_mode-选项的表来避免更新数据)保留重复行，不做合并。通过使用 MySQL 的客户端库（JDBC + 某个 MySQL
...
下面的命令会把表中数据全部导出，其中也包含双写已经送到 GreptimeDB 的那部分。只有目标表能按主键和
时间索引去重时，才可以直接照用。否则请先把 dump 导入中转表，只取早于 cutoff 的行；或者在导出时加上
时间列的 `--where` 条件——注意 `--where` 会作用于 dump 中的每一张表，不含该列的表需要单独导出。
