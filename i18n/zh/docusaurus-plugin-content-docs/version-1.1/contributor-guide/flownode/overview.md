---
keywords: [持续聚合, flow 管理, 单机模式, Flownode 组件, Flownode 限制]
description: Flownode 概览，一个为数据库提供 Flow 计算能力的组件，包括 batching mode、已废弃的 streaming mode 和核心组件。
---
...
- **Task Executor**: flow 逻辑执行的运行时环境。在批处理模式下，它是一个 `BatchingTask`；在已废弃的流处理模式下，这通常是一个 `FlowWorker`。
