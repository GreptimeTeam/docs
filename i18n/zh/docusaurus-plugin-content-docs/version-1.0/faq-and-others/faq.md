...
通过 InfluxDB Line Protocol、Prometheus Remote Write 等协议写入时，GreptimeDB 默认使用 `greptime_value` 和 `greptime_timestamp` 作为 Field 列和时间索引列的名称。
