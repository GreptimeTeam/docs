...
description: 介绍 GreptimeDB 双活互备架构下的 DR 方案，重点说明双向同步写入、本地查询执行和故障切换方式。
---
...
- 写入采用双向异步复制，任一节点接收到写请求后都会同步到对端节点。
  - GreptimeDB 以特有的代码架构解决了循环复制问题。
- 查询在各自节点本地执行，不需要跨节点合并查询结果。

该模式的目标是：在不引入第三个计算节点的前提下，以较低架构复杂度获得跨节点容灾能力。
...
若该空间大小为 0，则异步复制退化为同步复制，RPO 为 0。
