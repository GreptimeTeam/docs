...
默认情况下，导入的用户会获得 `public` schema 的完整访问权限
(`AclType::All`)。
...
ACL 在 schema 内提供表级安全控制。每条 ACL 都限定在某个 schema 中，并控制
用户可以访问该 schema 中的哪些表。

`all` ACL 授予对该 schema 中所有表的访问权限。当用户需要读取或写入该
schema 中当前和未来的所有表时，可以使用该 ACL；实际访问能力仍受用户全局
权限的约束。
...
`sensor_1`、`sensor_2024` 等表名。Regex ACL 会在配置的 schema 内对表名
