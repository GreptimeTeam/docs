...
**采集器和仪表盘不用换。** 写入侧支持 [Prometheus remote write](/user-guide/ingest-data/for-observability/prometheus.md) 和 [OTLP](/user-guide/ingest-data/for-observability/opentelemetry.md)，查询侧支持 [PromQL](/user-guide/query-data/promql.md) 和 [Jaeger 接口](/user-guide/query-data/jaeger.md)，两者各有独立的 HTTP 端点；SQL 则通过 [MySQL](/user-guide/protocols/mysql.md) 与 [PostgreSQL](/user-guide/protocols/postgresql.md) 线协议访问，[Grafana](/user-guide/integrations/grafana.md) 仪表盘可以继续使用。各协议的覆盖范围和限制见[协议](/user-guide/protocols/overview.md)。
