...
这张表是计算出来的，不是存储的。扫描它时，会在查询时间窗口内，从所有贡献了实体声明的表读时派生出行——声明既可以来自 `greptime.semantic.entity.*` 选项，也可以由内置约定解析得到。通过内置约定，OTLP trace 表、`target_info` 和 kube-state-metrics 描述性指标无需携带对应实体的显式 `greptime.semantic.entity.*` 选项，也能贡献约定定义的实体。它是只读的：`INSERT`、`CREATE`、`ALTER`、`TRUNCATE`、`DROP` 都会被拒绝。
