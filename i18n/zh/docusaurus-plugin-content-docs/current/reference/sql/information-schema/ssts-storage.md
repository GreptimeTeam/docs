...
`SSTS_STORAGE` 表提供直接从存储层列出的 SST（排序字符串表）文件信息。此表显示来自对象存储的原始文件元数据，可能包括尚未反映在 manifest 中的文件或已孤立的文件。
...
- **存储验证**：将存储中的文件与 manifest 进行比较，以检测孤立文件或不一致性。
- **存储调试**：识别存在于存储中但可能未在 manifest 中正确跟踪的文件。
...
查找存储中但不在 manifest 中的文件（潜在的孤立文件）：
...
|------|---------------|--------------|
| **数据源** | Manifest 元数据 | 直接从存储层 |
| **信息** | 详细的 SST 元数据（行数、时间范围等） | 仅基本文件元数据 |
| **文件覆盖** | 仅 manifest 中跟踪的文件 | 存储中的所有文件 |
| **使用场景** | 查询 SST 元数据进行分析 | 验证存储、查找孤立文件 |
| **性能** | 快速（从 manifest 读取） | 较慢（扫描存储） |
