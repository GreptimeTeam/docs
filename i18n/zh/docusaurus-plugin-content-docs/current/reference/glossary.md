...
### Flow Engine (Flow 引擎)
GreptimeDB 的持续聚合引擎，支持对持续写入的时序数据进行增量计算。Flow Engine 类似智能物化视图，当源表有新数据到达时自动更新结果表。聚合和 TQL workload 使用 batching mode；原始的 streaming mode 已经废弃，不推荐新 workload 使用。
