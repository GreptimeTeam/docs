...
### Flow Engine (流处理引擎)
GreptimeDB 的流处理组件，支持连续查询和实时数据聚合。Flow Engine 对到达的数据流进行实时处理，实现对 metrics、logs 和 traces 的实时分析和事件驱动计算。
...
### Pipeline (数据管道)
GreptimeDB 的 ETL（提取、转换、加载）功能，支持数据到达时的实时转换。Pipeline 使用户能够在存储前清理、丰富和重构可观测性数据，特别适用于日志处理和数据规范化。
