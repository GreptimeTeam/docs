...
正常来说，一次 Remote write 的请求会以同样的选项被写入到数据库中（例如：`db_name`）。
对于高级用户或者大量数据而言，将这些数据在写入的时候就划分存储位置，可以提高查询效率和速度。

从 `v0.15` 开始，GreptimeDB 将支持特殊标签。
这些标签（和它们的值）将会在解析阶段被转换为写入选项，使得一个请求中的不同指标可以更精细地被控制。

这些标签互不排斥，所以可以通过组合的方式达成更多样化的控制。

注意，如果你在使用 GreptimeDB 处理指标时未遇到任何问题，或者不太理解这些选项的作用，那么你很可能根本不需要它们。

### `__database__`

这个选项决定指标会被保存到哪个库中。注意，这个库需要被提前创建（例如使用 `create database xxx` SQL 进行创建）。

将同一个指标保存到不同的库中，可以提高带标签的查询速度。

### `__physical_table__`

如果指标通过 metric engine 进行存储，那么每一个指标的逻辑表背后都有一个物理表。
默认情况下，物理表是一张大宽表；但是你可以指定指标关联到不同的物理表上。

注意，指标的逻辑表在创建时就与物理表一一关联。在一个数据库下为一个指标设定不同的物理表不会生效。
但是，通过与上述 `__database__` 选项结合，这种方式就变成了可行的。

将指标保存到不同的物理表中可以提高查询效率。
