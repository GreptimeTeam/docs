...
一般来说，一次 Remote write 请求的全部数据会以同样的配置项被写入到数据库中，例如，如果开启了 metric engine 的话，一个同样的默认物理表。
即使指标的数量在增长，所有的逻辑表（即指标们）都会保存到同一张物理表中。
对于写入这可能是没问题的。但是对于需要查询一小部分指标的场景，这种设置可能会拖慢查询速度，因为所有的指标都聚集在一张物理表上，而数据库需要对全表的数据进行扫描。

如果你可以预见大量的数据写入和每次只需查询小部分指标的场景，那么可能在写入时对存储位置进行划分可能会减缓后续查询的压力。
...
以下是一个指标的特殊标签的代表图例。注意，这不是一个指标的实际数据模型。
...
这些上述的特殊标签只是在 Prometheus 中的普通有效标签。
...
这个选项决定指标数据会被保存到哪个库中。注意，这个库需要被提前创建好（例如通过 `create database xxx` SQL 语句）。

同样的技术栈会产生同样名称的指标。
假设你有两个 Kubernetes 集群，但是运行了不同的应用，然后通过一个指标收集工具同时收集两个集群的指标。
两个集群会产生名称一样但是标签和值完全不同的指标。
如果这些指标被写入到同一个库中，那么在使用 Grafana 大盘浏览这些指标的时候，就需要对大盘的每个图表手动设置标签值，才能区分集群进行查看。
这样既繁琐又低效。

在这种情况下，你可以在写入时将指标保存到两个不同的库中，然后使用两个大盘分别浏览这些指标。
...
在全量指标数据集中查找一个特定的指标或者标签将会消耗大量时间，因为数据库需要扫描所有“不相关”的数据。

这种场景下，将指标划分成不同的物理表可以减轻单一物理表的压力。将指标通过写入频率来聚集也是有帮助的。

注意，指标的逻辑表在创建时就与物理表一一关联。在一个数据库下为一个指标设定不同的物理表不会生效。
