...
description: 使用 Flow 为日志分析、传感器监控和仪表盘维护时间窗口聚合结果。
---

# 持续聚合

Flow 会随 source 数据变化维护聚合结果。常见用途包括对高频数据降采样、维护监控阈值结果，以及预计算仪表盘查询。下面分别给出示例。
...
然后创建 `ngx_aggregation`，按一分钟窗口和状态码计算请求数量及大小统计。`high_size_count` 统计 `size` 大于 550 的行数。
...
`ngx_country` 表按窗口保存不同的国家。`EXPIRE AFTER` 会排除 `access_time` 早于七天前的 source 数据，详见[管理 Flow](manage-flow.md#expire-after)。
...
这个例子按十秒窗口维护每个传感器和位置的最高温度，并只保留最高温度超过 100 的窗口：
...
它计算每个传感器和位置的最大温度读数，并保留最大温度超过 100 度的结果。
`temp_alerts` 表将不断更新聚合数据，
当温度超过阈值时在 `temp_alerts` 表中生成告警行。`EXPIRE AFTER '1h'` 将 Flow 使用的 source 数据限制为最近一小时，详见[管理 Flow](manage-flow.md#expire-after)。
