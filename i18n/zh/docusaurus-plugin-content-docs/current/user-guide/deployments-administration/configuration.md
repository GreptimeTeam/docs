...
|         | copy_root         | 字符串 | 单机模式下 SQL 访问本地文件的根目录（默认为 `<data_home>/copy`）。`COPY` 语句和外部表中的相对路径将在该目录下解析；仅当绝对路径位于该目录内时才被接受。分布式部署始终禁止 SQL 访问本地文件。升级指引请参阅[迁移本地 SQL 文件访问](./migrate-local-sql-file-access.md)。 |
