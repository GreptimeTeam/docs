...
两个独立节点都能提供服务，并通过本地持久化队列异步复制数据变更请求。
对端或站点间网络故障时，健康节点可以继续服务，并在对端恢复后回放队列请求。

只有源节点的队列磁盘仍然可用时，队列才能保护已确认写入。如果复制完成前同时丢失节点及其队列磁盘，对端可能缺少仍在队列中的请求。
...
|  基于双活互备的 DR 解决方案 | 跨区域 | 取决于待处理队列和故障类型 | 取决于外部故障切换 | 低 | 中小型场景中对可用性和可靠性要求较高 |  可选 | 商业功能 |
