...
`greptime_private.events` 表记录 GreptimeDB 运行期间产生的事件。可以用它检查 DDL 变更
和后台 Procedure，无需翻查服务日志。
...
单机部署在本地配置事件记录器，分布式部署在 Metasrv 上配置。配置选项和支持的事件类型，
请参阅[生命周期事件记录器](/user-guide/deployments-administration/configuration.md#生命周期事件记录器)。
