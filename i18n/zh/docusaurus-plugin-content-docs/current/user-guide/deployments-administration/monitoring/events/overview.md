---
keywords: [GreptimeDB 事件, 生命周期事件, 运维事件, greptime_private]
description: 查询 greptime_private.events 中记录的生命周期和运维事件。
---

# 事件

`greptime_private.events` 表保存生命周期事件和运维事件，帮助运维人员检查 DDL 变更和后台 Procedure，而不必依赖服务日志。
