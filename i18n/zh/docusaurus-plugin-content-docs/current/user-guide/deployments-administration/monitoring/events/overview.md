...
事件记录器首次写入事件时会创建 `greptime_private.events` 系统表。如果现有表缺少当前事件表结构中的列，记录器会补齐这些列。即使自动建表已禁用，仍会执行这两项操作。禁用事件记录不会删除已有表。只有该表从未创建过时，下面的查询才会报表不存在，例如从启动时就禁用了事件记录，因而没有记录任何事件。
