...
- `ADMIN purge_table("table_name")` 解析的是 tombstoned table，而不是同名 live table。

## 自动 purge

Metasrv GC 会周期性扫描 soft-dropped table。当某个表的 `retention_expires_at` 已过期后，GC 会提交与 `ADMIN purge_table()` 相同的 purge procedure。Retention deadline 在执行 drop 时固定，之后修改 `gc.experimental_soft_drop.retention` 不会影响已经删除的表。
