...
从 PostgreSQL 中导出元数据到 s3 。此命令会导出到 `test-bucket` 的 `metadata_snapshot.metadata.fb` 文件中：
...
将导出的元数据从 s3 导入到 PostgreSQL 存储后端。此命令会导入 `test-bucket` 的 `metadata_snapshot.metadata.fb` 文件中的数据：
