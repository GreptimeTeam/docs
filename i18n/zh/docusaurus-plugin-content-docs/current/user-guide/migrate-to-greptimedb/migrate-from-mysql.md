...
因此需要记录失败的写入、对两个目标分别重试，并在切流前完成对账。通过使用 MySQL 的客户端库（JDBC + 某个 MySQL
...
下面的命令导出的是整张表，包含双写已经送到 GreptimeDB 的那部分数据。只有当目标表能按主键和
时间索引去重时才可以直接照用。否则请先把 dump 导入中转表，只搬运早于 cutoff 的行；或者给导出加上
时间列的 `--where` —— 注意 `--where` 会作用于 dump 中的每一张表，不含该列的表需要单独导出。
