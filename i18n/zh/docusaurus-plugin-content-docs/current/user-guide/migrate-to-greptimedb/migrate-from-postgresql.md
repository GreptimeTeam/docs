...
因此需要记录失败的写入、对两个目标分别重试，并在切换前完成对账。重放前请先确认该行确实缺失——[append-only 表](/user-guide/manage-data/overview.md#通过创建带有-append_mode-选项的表来避免更新数据)会保留重复行，不会合并。通过使用 PostgreSQL 的客户端库（JDBC + 某个 PostgreSQL
