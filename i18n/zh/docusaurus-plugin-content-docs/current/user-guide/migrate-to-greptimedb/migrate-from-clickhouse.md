...
-   记录每一次失败的写入，保留足以重放的上下文，并对两个目标分别重试。
-   确定一个 cutoff（时间戳或数据源位点），划清历史数据导出与双写各自覆盖的范围，避免两者重叠或中间留下空档。
-   切流前做对账：按时间窗口比较行数与聚合值，而不是默认两边一致。

历史数据导出与导入
---------

1.  **先开启双写，再确定 cutoff** 让应用同时写入 ClickHouse 和 GreptimeDB，并记下双写生效的那一刻。把**这一刻本身**作为 cutoff `C`，下面所有导出都以它为界：历史导出覆盖 `timestamp < C`，`C` 之后由双写覆盖。

    不要选一个晚于双写起点的 cutoff：这中间写入的行会既被双写送达、又被导出命中，除非目标表能按主键和时间索引去重，否则会写入两次。该切分方式还假设每行的时间戳不早于它被写入的时刻；迟到或回填的数据若带着更早的时间戳，仍会两边都落一份。
