...
-   记录每一次失败的写入，保留足以重放的上下文，并对两个目标分别重试。重放前请先确认该行确实缺失：[append-only 表](/user-guide/manage-data/overview.md#通过创建带有-append_mode-选项的表来避免更新数据)会保留重复行，而不是按主键和时间索引合并，因此重试一次实际已经写入成功的请求会留下两份数据。
