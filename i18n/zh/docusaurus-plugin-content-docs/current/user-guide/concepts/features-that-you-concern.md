...
通过 SQL 建表时，默认的 `last_row` 保留最后一行；`last_non_null` 分别保留每个 Field 最后写入的非空值。自动建表可能采用写入协议自己的默认配置。去重只在同一个 Region 内生效，因此去重表的分区列必须是主键列的子集。
...
- `flat` SST 格式减少按序列维护的开销，是新表的默认格式，适合高基数主键。
