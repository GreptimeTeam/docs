...
[Agent RCA Bench](https://rca-bench.greptime.com/#zh) 完成了 504 次端到端排查——6 个模型、14 个故障、每组重复两次——覆盖三种接口：Prometheus、Loki、Tempo 各自的原生查询 API；GreptimeDB 表之上的只读 SQL 与 PromQL；以及在此基础上再加一层[语义层](./semantic-layer.md)。
