...
| `--output-dir`            | 是（未配置远端存储时） | - | 导出文件使用的目录。未配置远端对象存储时必须设置。**重要：**导出数据时，该路径由 GreptimeDB Server 解析；schema SQL 文件由 CLI 进程写入。如果 CLI 与 Server 不在同一主机，请使用远端对象存储，并通过 `--ddl-local-dir` 指定 SQL 文件的本地目录。                                                                                          |
...
| `--s3-access-key-id`      | 是\*     | -          | 当设置了 `--s3` 时，需指定 S3 的 Access Key ID                                                                                                                                                                                                                             |
| `--s3-secret-access-key`  | 是\*     | -          | 当设置了 `--s3` 时，需指定 S3 的 Secret Access Key                                                                                                                                                                                                                         |
