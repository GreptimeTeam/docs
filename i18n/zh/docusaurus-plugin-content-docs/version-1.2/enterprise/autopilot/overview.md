...
Autopilot 是 GreptimeDB Enterprise 中用于自动优化集群负载和数据分布的能力。它运行在 Metasrv 中，通过持续收集 Datanode 和 Region 的写吞吐和查询 CPU 时间统计信息，在满足条件时自动提交调度动作，减少人工排查热点和手动调整集群的运维成本。

Autopilot 当前包括以下能力：

- **Region Balancer**：自动迁移热点 Region，使 Datanode 之间的写入负载和读查询 CPU 负载更均衡。
...
- **运行时**：按照固定间隔触发一次调度周期。
- **集群统计信息**：通过 Datanode heartbeat 收集 Region 写吞吐和查询 CPU 时间统计信息，并对短期波动进行平滑。
...
- 集群中部分 Datanode 的写入负载或读查询 CPU 负载长期明显高于其他 Datanode；
...
- `plugins.cluster_stat`：配置 Datanode 和 Region 写吞吐和查询 CPU 时间统计信息的采样和平滑方式。
...
- 如需自动平衡 Datanode 之间的写入负载和读查询 CPU 负载，请参考 [Region Balancer](./region-balancer.md)。
