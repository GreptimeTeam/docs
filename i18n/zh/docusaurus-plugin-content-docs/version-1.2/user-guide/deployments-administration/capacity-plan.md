...
GreptimeDB 会对落盘数据做压缩，落盘数据通常约为原始数据的 1/8 到 1/10。
实际压缩率取决于表结构、列类型以及数据本身的重复程度。
...
使用云存储时，本地磁盘主要用于查询相关的缓存和 Write-Ahead Log (WAL)，一般 200GB 左右就够用。
...
这套配置是否成立，取决于表结构、平均行宽、索引与 TTL 配置，以及 200 QPS 背后查询的时间范围和并发度。
