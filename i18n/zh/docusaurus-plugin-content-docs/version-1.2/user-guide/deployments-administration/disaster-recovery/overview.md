...
此解决方案面向节点或 AZ 故障场景下的零 RPO 和秒级或分钟级 RTO。和其他方案一样，这些指标取决于你需要逐项确认的前提：
...
此解决方案的目标是 Region 级别的容错、可扩展的写入能力、零 RPO 以及秒级或分钟级的 RTO。能否达到这些指标取决于整条依赖链，而不只是集群的部署形态：
...
|  基于单集群单区域部署的 DR 解决方案 | 单区域，节点与 AZ 级 | 0 | 秒级或分钟级 | 中 | 部署在单个区域内的集群，常见的生产基线 | 必需 | Region Failover 默认关闭 |
|  基于双活互备的 DR 解决方案 | 跨区域 | 取决于待同步的数据变更和故障类型 | 取决于外部故障切换 | 低 | 中小型场景中对可用性和可靠性要求较高 |  可选 | 商业功能 |
|  基于单集群跨区域部署的 DR 解决方案 | 多区域 | 0 | 秒级或分钟级 | 高 | 中大型场景中对可用性和可靠性要求较高 |  必需 | |
