...
如果目标后端非空且**未**传 `--force`，命令只会打印一条警告便退出，不恢复任何数据，而退出码仍为成功。因此必须查看命令输出，不能想当然地认为恢复已经完成。

`--force` 只会跳过非空检查，不会清空目标后端。恢复时，快照中的键会直接写入，其他已有键仍会保留，因此可能导致元数据不一致。建议恢复到一个空的目标后端，并在重启集群后检查 catalog、表路由和 Procedure 状态。
:::
