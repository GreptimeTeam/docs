...
- [gRPC SDK](./grpc-sdks/overview.md)——提供 Go、Java、Rust、.NET、Erlang 和 TypeScript 客户端。
- [EMQX](emqx.md)——MQTT 设备，通过 EMQX 的数据集成写入。
- [InfluxDB 行协议](influxdb-line-protocol.md)——从 InfluxDB 迁移时走这条路径，也适用于 Telegraf，该页同时包含 Telegraf 的配置方式。
- [OpenTSDB](opentsdb.md)——通过 `/v1/opentsdb/api/put` HTTP 接口写入。
