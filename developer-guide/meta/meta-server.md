...
- Request-Router, so the Frontend knows where the data should be written to and read from.
...
- Election & High Availability, GreptimeDB is designed as Leader-Follower architecture, only Leader nodes can write, Follower nodes can provide read, the number of Follower nodes is usually >= 1, Follower nodes need to be able to switch to Leader quickly when Leader is not available.
