...
A flow stores its aggregated data in a sink table. `CREATE FLOW` creates the sink table if it does not exist,
so defining it yourself is optional — do it when you want control over the schema, or when the table already exists.
An existing sink table must be compatible with the flow's query result, which means:
