...
A flow stores its aggregated data in a sink table. `CREATE FLOW` creates that table when it does not exist,
so defining it yourself is optional — do it when you want control over the schema, for example to choose the
primary key or set a TTL. Either way, the sink table has to be compatible with the flow's query result:
