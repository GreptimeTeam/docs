...
:::note
The index sizes and the multipliers in this section come from Greptime's own
testing on a log dataset. The test setup was not published, so use them to
compare the two backends against each other, not as absolute figures for your
data. Selectivity, term distribution, and cache state all move these numbers.
:::
