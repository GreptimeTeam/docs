...
| `--rpc-server-addr <RPC_SERVER_ADDR>` | Address advertised to Frontend, Datanode, and Flownode. When unset, GreptimeDB uses the first network interface address and the port from `rpc_bind_addr` |
...
The [`metasrv.example.toml`](https://github.com/GreptimeTeam/greptimedb/blob/v1.0.2/config/metasrv.example.toml) file is in the GreptimeDB repository. The `-c` option selects the configuration file; see [Configuration](/user-guide/deployments-administration/configuration.md) for details.
